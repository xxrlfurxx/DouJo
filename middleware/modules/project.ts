...
// 숫자 페이징에서 추가할 때
export const requestAddProjectPaging = createAction<ProjectItem>(
  `${projectReducer.name}/requestAddProjectPaging`
);
...
export const requestRemoveProjectPaging = createAction<number>(
  `${projectReducer.name}/requestRemoveProjectPaging`
);
...
function* addDataPaging(action: PayloadAction<ProjectItem>) {
  ...
  try {
    ...
    const projectItemRequest: ProjectItemRequest = {
      ...
      // title: "", // 임시로 에러 유발(400)
      description: projectItemPayload.description,
      // photoUrl: photoItemPayload.photoUrl, // base64 dataURL
      photoUrl: project, // 응답받은 Cloudfront URL로
      fileType: projectItemPayload.fileType,
      fileName: projectItemPayload.fileName,
    };
...
function* addDataNext(action: PayloadAction<PhotoItem>) {
  ...
  try {
    ...
    const projectItemRequest: ProjectItemRequest = {
      ...
      enddate: photoItemPayload.enddate,
      manager: photoItemPayload.manager,
      enginner: photoItemPayload.enginner,
      milestone: photoItemPayload.milestone,
    };
...
function* fetchPagingData(action: PayloadAction<PageRequest>) {
  ...
  localStorage.setItem("project_page_size", size.toString());
  ...
  try {
    ...
    // state 초기화 reducer 실행
    yield put(initialPagedProject(projectPage));
  } catch (e: any) {
    // 에러발생
    // spinner 사라지게 하기
    yield put(endProgress());
...
function* removeDataPaging(action: PayloadAction<number>) {
  ...
  // 현재 페이지 데이터를 다시 가져옴
  // 현재 페이지와 사이즈 값을 읽어옴
  const page: number = yield select((state: RootState) => state.project.page);
  const size: number = yield select((state: RootState) => state.project.pageSize);

  yield put(requestFetchPagingProject({ page, size }));
}
...
export default function* projectSaga() {
  ...
  // 동일한 타입의 액션은 모두 처리함
  yield takeEvery(requestAddProject, addDataNext);
  yield takeEvery(requestAddProjectPaging, addDataPaging);
  yield takeEvery(requestAddProjectNext, addDataNext);

  // takeLatest(처리할액션, 액션을처리할함수)
  // 동일한 타입의 액션중에서 가장 마지막 액션만 처리, 이전 액션은 취소

  // 1건의 데이터만 조회
  yield takeEvery(requestFetchProjectItem, fetchDataItem);
  yield takeLatest(requestFetchProjects, fetchData);
  yield takeLatest(requestFetchPagingProjects, fetchPagingData);
  yield takeLatest(requestFetchNextProjects, fetchNextData);

  // 삭제처리
  yield takeEvery(requestRemoveProject, removeDataNext);
  yield takeEvery(requestRemoveProjectPaging, removeDataPaging);
  yield takeEvery(requestRemoveProjectNext, removeDataNext);
